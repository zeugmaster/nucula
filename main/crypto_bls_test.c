...
static int test_bls_suite(void)
{
    ...
    /* Multi-key batch: every amount has a DISTINCT mint key within a keyset,
     * so real tokens batch across many keys. Keys 2*g2, 3*g2, 2*g2 — the
     * non-adjacent repeat also exercises the last-validated-K cache miss.
     * (K3 = 3*g2 precomputed on the host; C_i = a_i*Y_i built here in G1.) */
    {
        static const char *K3_HEX =
        ...
        unsigned char ks3[3 * 96], cs3[3 * 48];
        hex_to_bytes(V3_K_HEX, ks3, 96);
        hex_to_bytes(K3_HEX, ks3 + 96, 96);
        hex_to_bytes(V3_K_HEX, ks3 + 192, 96);
        const unsigned char *sec3[3] = {
            (const unsigned char *)"multikey_1",
            (const unsigned char *)"multikey_2",
            (const unsigned char *)"multikey_3",
        };
        const size_t slen3[3] = { 10, 10, 10 };
        const unsigned char a_vals[3] = { 2, 3, 2 };
        blst_hw_acquire();
        for (int i = 0; i < 3; i++) {
            blst_scalar a;
            small_scalar(&a, a_vals[i]);
            blst_p1 y, c;
            hash_to_g1(&y, sec3[i], slen3[i]);
            p1_mul(&c, &y, &a);
            blst_p1_compress(cs3 + i * 48, &c);
        }
        blst_hw_release();
        if (s->verify_proofs(NULL, 3, ks3, cs3, sec3, slen3) != 1) {
            ESP_LOGE(TAG, "suite verify_proofs multi-key: rejected a valid batch");
            pass = 0;
        }
        const unsigned char *bad3[3] = {
            sec3[0], (const unsigned char *)"multikey_x", sec3[2],
        };
        if (s->verify_proofs(NULL, 3, ks3, cs3, bad3, slen3) != 0) {
            ESP_LOGE(TAG, "suite verify_proofs multi-key: accepted a tampered batch");
            pass = 0;
        }
        /* Key/proof mismatch: proof 0's C under proof 1's key must fail. */
        unsigned char ks_swapped[3 * 96];
        memcpy(ks_swapped, ks3 + 96, 96);
        memcpy(ks_swapped + 96, ks3, 96);
        memcpy(ks_swapped + 192, ks3 + 192, 96);
        if (s->verify_proofs(NULL, 3, ks_swapped, cs3, sec3, slen3) != 0) {
            ESP_LOGE(TAG, "suite verify_proofs multi-key: accepted swapped keys");
            pass = 0;
        }
    }
    if (pass)
        ESP_LOGI(TAG, "suite verify_proofs (incl. multi-key): OK");
 ...
 * mint key, as in real tokens (each amount has its own key). Suite ops
 * manage the MPI lock themselves, so these rows run OUTSIDE any hold window
 * (the lock is not recursive). Brief yields between stages inside the timed
 * body feed the 5 s idle watchdog; they add ~3 ticks to a multi-second row. */
static void bench_suite_swap(void)
{
    ...
    /* Setup (mint-side, untimed): input proofs C_i = a_i*Y_i and blind
     * signatures C__i = a_i*B__i over the outputs — G1 only. */
    blst_hw_acquire();
    for (int i = 0; i < N; i++) {
        int n1 = snprintf(in_secret_bufs[i], sizeof(in_secret_bufs[i]), "swap_in_%d", i);
        int n2 = snprintf(out_secret_bufs[i], sizeof(out_secret_bufs[i]), "swap_out_%d", i);
        in_secrets[i] = (const unsigned char *)in_secret_bufs[i];
        in_lens[i] = (size_t)n1;
        out_secrets[i] = (const unsigned char *)out_secret_bufs[i];
        out_lens[i] = (size_t)n2;

        hex_to_bytes(BENCH_K_HEX[i], Ks + i * 96, 96);

        blst_scalar a;
        small_scalar(&a, (unsigned char)(2 + i));

        blst_p1 y, c;
        hash_to_g1(&y, in_secrets[i], in_lens[i]);
        p1_mul(&c, &y, &a);
        blst_p1_compress(Cin + i * 48, &c);

        blst_p1 yo, b, cb;
        blst_scalar r;
        small_scalar(&r, 3);
        hash_to_g1(&yo, out_secrets[i], out_lens[i]);
        p1_mul(&b, &yo, &r);
        p1_mul(&cb, &b, &a);
...
void crypto_bls_run_benchmark(void)
{
    ...
    bench_rows();
    blst_hw_release();
    /* Production-suite swap rows: MPI pass only (the portable pass would run
     * ~20 s of pairings for a column nobody ships). Outside the hold window —
     * the suite ops acquire the non-recursive lock themselves. */
    bench_suite_swap();
#else
    bench_rows();
    bench_suite_swap();
